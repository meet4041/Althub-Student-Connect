...
// --- INJECTED STYLES FOR MODERN REGISTER UI ---
const styles = `
  ...
  .profile-preview {
    ...
    border-radius: 50%;
    object-fit: cover;
    border: 4px solid #f0f9f6;
    box-shadow: 0 5px 15px rgba(0,0,0,0.1);
  }

  .upload-label {
    padding: 10px 20px;
    border: 1px dashed #66bd9e;
    color: #66bd9e;
    border-radius: 8px;
    cursor: pointer;
    font-weight: 500;
  ...
  @media (max-width: 900px) {
    .register-page-wrapper { flex-direction: column; }
    .reg-visual-section { padding: 40px 20px; flex: none; }
    .reg-form-section { padding: 30px 20px; }
...
export default function Register() {
  ...
  const colorStyle = {
    ...
    option: (styles, { isFocused, isSelected }) => ({
      ...styles,
      backgroundColor: isSelected ? "#66bd9e" : isFocused ? "#f0f9f6" : null,
  ...
  const validateStep = (currentStep) => {
    ...
    if (currentStep === 1) {
      ...
      if (!input["phone"]) { 
        isValid = false; stepErrors["phone_err"] = "Phone is required"; 
      } else if (!validatePhone(input["phone"])) {
        isValid = false; stepErrors["phone_err"] = "Enter 10 digit number";
      }
      if (!input["email"]) { 
        isValid = false; stepErrors["email_err"] = "Email is required"; 
      } else if (!validateEmail(input["email"])) {
        isValid = false; stepErrors["email_err"] = "Invalid email format";
      }
    }
      ...
      <div className="reg-visual-section">
        ...
        <div className="reg-visual-content">
            ...
            <div className="login-redirect">
                Already a member? <a href="#" onClick={(e) => {e.preventDefault(); nav("/login")}}>Log In</a>
            </div>
        </div>
        <img src="/images/Usability testing-bro.png" alt="Register Visual" className="reg-img" />
      </div>
        ...
        <div className="reg-form-container">
                ...
                <div>
                    ...
                    <div className="input-grid">
                        <div className="form-group">
                            <input type="text" name="fname" className="form-control" placeholder="First Name" value={user.fname} onChange={handleChange} />
                            {errors.fname_err && <span className="error-text">{errors.fname_err}</span>}
                        </div>
                        <div className="form-group">
                            <input type="text" name="lname" className="form-control" placeholder="Last Name" value={user.lname} onChange={handleChange} />
                            {errors.lname_err && <span className="error-text">{errors.lname_err}</span>}
                        </div>
                    </div>

                    <div className="form-group">
                        <label style={{fontSize: '0.9rem', color: '#666'}}>Date of Birth</label>
                        <input type="date" name="dob" className="form-control" value={user.dob} onChange={handleChange} />
                        {errors.dob_err && <span className="error-text">{errors.dob_err}</span>}
                    </div>

                    <div className="form-group">
                        <label style={{fontSize: '0.9rem', color: '#666'}}>Gender</label>
                        <div className="gender-group">
                            <div className={`gender-option ${user.gender === "Male" ? "selected" : ""}`} onClick={() => {setUser({...user, gender: "Male"}); setErrors(p=>({...p, gender_err: ""}))}}>
                                <i className="fa-solid fa-mars"></i> Male
                            </div>
                            <div className={`gender-option ${user.gender === "Female" ? "selected" : ""}`} onClick={() => {setUser({...user, gender: "Female"}); setErrors(p=>({...p, gender_err: ""}))}}>
                    ...
                    <div className="input-grid">
                        <div className="form-group">
                            <input type="text" name="phone" className="form-control" placeholder="Phone Number" value={user.phone} onChange={handleChange} />
                            {errors.phone_err && <span className="error-text">{errors.phone_err}</span>}
                        </div>
                        <div className="form-group">
                            <input type="text" name="email" className="form-control" placeholder="Email Address" value={user.email} onChange={handleChange} />
                            {errors.email_err && <span className="error-text">{errors.email_err}</span>}
                        </div>
                    </div>

                    <div className="btn-group">
                        <button className="btn-next" onClick={increaseStep}>Next Step <i className="fa-solid fa-arrow-right"></i></button>
                    </div>
                </div>
            )}
            ...
            {step === 2 && (
                <div>
                    ...
                    <div className="btn-group">
                        <button className="btn-prev" onClick={decraseStep}>Back</button>
                        <button className="btn-next" onClick={increaseStep}>Next Step</button>
                    </div>
                </div>
            )}
            ...
            {step === 3 && (
                <div>
                    ...
                    <div className="form-group">
                        <Select
                            options={university.map(u => ({label: u.name, value: u.name}))}
                            onChange={(opt) => {
                                setUser({...user, institute: opt.value});
                                setErrors(p=>({...p, institute_err: ""}));
                            }}
                            placeholder="Select Institute"
                            styles={colorStyle}
                        />
                        {errors.institute_err && <span className="error-text">{errors.institute_err}</span>}
                    </div>

                    <div className="form-group">
                        <Select options={option1} isMulti onChange={handleSelect1} placeholder="Languages Known" value={languages} styles={colorStyle} />
                        {errors.languages_err && <span className="error-text">{errors.languages_err}</span>}
                    </div>

                    <div className="form-group">
                        <Select options={option2} isMulti onChange={handleSelect2} placeholder="Skills" value={skills} styles={colorStyle} />
                        {errors.skills_err && <span className="error-text">{errors.skills_err}</span>}
                    </div>

                    <div className="input-grid">
                        <div className="form-group">
                            <input type="text" name="city" className="form-control" placeholder="City" value={user.city} onChange={handleChange} />
                            {errors.city_err && <span className="error-text">{errors.city_err}</span>}
                        </div>
                        <div className="form-group">
                            <input type="text" name="state" className="form-control" placeholder="State" value={user.state} onChange={handleChange} />
                            {errors.state_err && <span className="error-text">{errors.state_err}</span>}
                        </div>
                    </div>
                    <div className="form-group">
                        <input type="text" name="country" className="form-control" placeholder="Country" value={user.country} onChange={handleChange} />
                        {errors.country_err && <span className="error-text">{errors.country_err}</span>}
                    </div>
            ...
            {step === 4 && (
                <div className="profile-upload-box">
                    ...
                    {user.profilepic ? (
                        <img src={`${WEB_URL}${user.profilepic}`} alt="Preview" className="profile-preview" />
                    ) : (
                        <div className="profile-preview" style={{background: '#f0f0f0'}}></div>
                    )}
            ...
            {step === 5 && (
                <div>
                    ...
                    <div className="form-group">
                        <input type="password" name="password" className="form-control" placeholder="Create Password" value={user.password} onChange={handleChange} />
                        {errors.password_err && <span className="error-text">{errors.password_err}</span>}
                    </div>

                    <div className="form-group">
                        <input type="password" name="cpassword" className="form-control" placeholder="Confirm Password" value={user.cpassword} onChange={handleChange} />
                        {errors.cpassword_err && <span className="error-text">{errors.cpassword_err}</span>}
                    </div>
