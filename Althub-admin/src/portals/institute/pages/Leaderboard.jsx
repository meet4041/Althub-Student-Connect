                                    ...
                                    <table className="table table-hover mb-0">
                                        <thead>
                                            <tr className="admin-table-head-row">
                                                <th className="border-0 pl-4 py-3 leaderboard-rank-col">Rank</th>
                                                <th className="border-0 py-3">Member Identity</th>
                                                <th className="border-0 text-center py-3">Total Reviews</th>
                                                <th className="border-0 text-right pr-5 py-3">Quality Score</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {loading ? (
                                                ...
                                                <tr key={index} className="admin-hover-row">
                                                    <td className="pl-4 align-middle">
                                                        ...
                                                        {index === 2 && <span className="badge px-2 py-1 shadow-sm leaderboard-bronze">3rd</span>}
                                                        {index > 2 && <span className="text-muted font-weight-bold ml-2">{(index + 1).toString().padStart(2, '0')}</span>}
                                                    </td>
                                                    <td className="align-middle">
                                                        <div className="font-weight-bold text-dark admin-name-cell">{item.name}</div>
                                                    </td>
                                                    <td className="text-center align-middle">
                                                        <span className="badge badge-light border px-3 py-2 admin-muted-badge">
                                                            {item.totalFeedback} Entries
                                                        </span>
                                                    </td>
                                                    <td className="text-right pr-5 align-middle">
                                                        <span className={`leaderboard-score-pill ${item.averageRating >= 4 ? 'good' : item.averageRating >= 3 ? 'ok' : 'bad'}`}>
                                                            {item.averageRating.toFixed(1)} <i className="fa fa-star ml-2 leaderboard-star"></i>
                                                        </span>
                                                    </td>
                                                </tr>
                                            )) : (
